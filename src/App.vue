<template>
    <div class="weatherapp relative bg-green-500 text-white h-screen" >
      <div class=" flex justify-center " >
        <h1 class=" text-center mt-5 text-4xl mb-10 bg-gray-400  bg-opacity-25 p-4 rounded  text-green-100 "  >   Weather App</h1>
      </div>
      <div class="flex flex-col text-center justify-center">
        ...
        <button class="border rounded-md mt-2 mb-2 p-1 text-gray-500  hover:bg-gray-400 hover:text-gray-800  hover:p-1 " @click="searchClick" > Check Weather </button>
        </div>
...
export default {
  ...
  methods:{
      ...
      search(e){
           if ( e.key == "Enter"  ) {
                  fetch( `${this.base_url}weather?q=${this.query}&units=metric&appid=${this.api_key}`).then(response => response.json()).then( this.results); 
